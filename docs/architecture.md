...
Claude Code Runtime
  ...
  │   │
  │   ├─ [패턴 있으면] review-knowledge.md 파싱
  │   │   ├─ pending 패턴 제외
  │   │   ├─ git diff로 변경 파일 확인
  │   │   ├─ 파일 확장자 + 컨텍스트 키워드로 관련 패턴 필터링
...
/review-evolve ──→ 캡처 로드 (.review-evolve/captures/)
                     ...
                     ├──→ 사용자 인터랙티브 확인
                     │
                     └──→ .review-evolve/review-knowledge.md 업데이트
...
OMC team 실행 ──→ code-reviewer Agent/Task 생성
                 ...
                 PreToolUse 훅 발동
                 ├─ JSON 저장 지시 주입
                 └─ review-knowledge.md에서 관련 패턴 선별 주입
                        │
...
└── .review-evolve/
    ...
    └── review-knowledge.md     ← 축적된 리뷰 패턴 지식 베이스
```
...
|----------------|-------------------|------|
| Memory | review-knowledge.md — 패턴 지속 저장 | templates/review-knowledge.md |
