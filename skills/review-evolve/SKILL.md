...
## 축적된 리뷰 패턴 (자동 로드)
!`cat .review-evolve/review-knowledge.md 2>/dev/null || echo "아직 축적된 패턴 없음"`
...
1. 유효 판정된 이슈를 `.review-evolve/review-knowledge.md`에 추가한다.
   - 파일이 없으면 `templates/review-knowledge.md`를 복사하여 생성
2. 패턴 포맷:
   ```markdown
...
3. **중복 처리**: 기존 패턴과 의미적으로 동일한 경우:
   ...
   - confidence 승격 규칙 적용
4. 적절한 섹션에 패턴을 배치:
   - confidence: high → "## High Confidence Patterns" 섹션
   - confidence: medium → "## Medium Confidence Patterns" 섹션
   - confidence: pending → "## Pending Verification" 섹션
...
| 현재 상태 | 조건 | 변경 후 |
|-----------|------|---------|
| pending | 사용자가 유효 승인 | medium |
| medium | 2회 이상 재발견 또는 양쪽 모두 발견 | high |
| high | 5회 이상 발견 | core |
